...
client.on(Events.MessageCreate, async message => {
    ...
    webhooks.forEach(async webhook => {
        ...
        const prompt = `
        In this conversation you should act as ${webhook.name}. This should be a natural conversation, so please reply as briefly and concisely as possible, maximum 200 characters. Respond with only the content part, not the author. You may choose not to reply for any reason, but must reply if prompted to. In that case, output N/A as yourMessage. Here is the last part of the conversation. : ${JSON.stringify(context)}`
